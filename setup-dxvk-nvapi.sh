...
show-help() {
    ...
    echo
    echo "可用操作："
    echo "  install             安装 DXVK-NVAPI"
    echo "  uninstall           卸载 DXVK-NVAPI"
    echo
    echo "可用选项："
...
install-dll() {
    if [[ -n "$x64_dstdir" ]]; then
        if install-file "${1}64" "$replacements_x64" "$x64_dstdir"; then
            dll-override "${1}64"
        fi
    fi
...
uninstall-dll() {
    if [[ -n "$x64_dstdir" ]]; then
        if uninstall-file "${1}64" "$replacements_x64" "$x64_dstdir"; then
            dll-restore "${1}64"
        fi
    fi
    if [[ -n "$x86_dstdir" ]]; then
